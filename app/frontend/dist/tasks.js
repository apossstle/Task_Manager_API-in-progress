...
let allTasks = [];
let filters = { status: "all", priority: 0 };
// ─────────────────────────────────────────
// Utils
// ─────────────────────────────────────────
function el(id) {
...
async function createTask() {
    ...
    const descEl = el("task-desc");
    const prioEl = el("task-priority");
    ...
    if (dueEl.value)
        payload.due_date = new Date(dueEl.value).toISOString();
    try {
...
async function updateTaskStatus(id, currentStatus) {
    const payload = { status: STATUS_NEXT[currentStatus] };
    try {
...
function renderTasks(tasks) {
    ...
    <li class="task-item priority-${task.priority}" data-id="${task.id}">
      <div class="task-main">
        <div class="task-badges">
          ...
          <span class="badge badge-priority prio-${task.priority}">${PRIORITY_LABEL[task.priority]}</span>
  ...
  `).join("");
    list.querySelectorAll(".btn-cycle").forEach(btn => {
        btn.addEventListener("click", () => {
            updateTaskStatus(Number(btn.dataset.id), btn.dataset.status);
        });
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    el("task-title").addEventListener("keypress", (e) => {
        if (e.key === "Enter")
            createTask();
    });
    // Filter buttons
